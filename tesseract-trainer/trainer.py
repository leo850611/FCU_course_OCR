...
class TesseractTrainer():
    def __init__(self):
        ...
        self.fontName = "fcuFont"
        self.directory = "D:\code"
    ...
    def cleanImages(self):
        ...
        for fileName in files:
            if fileName.endswith("jpg") or fileName.endswith("jpeg") or fileName.endswith("png"):
                image = Image.open(self.directory+"/"+fileName).convert('L')
                
                #Set a threshold value for the image, and save           
                image = image.point(lambda x: 0 if x<130 else 255)
                image = image.point(lambda i: i * 1.1)
    ...
    def createFontFile(self):
        ...
        with open(fname, 'w') as fout:
            fout.write(self.fontName+" 0 1 0 0 0")
        os.chdir(currentDir)
